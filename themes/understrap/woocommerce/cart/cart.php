...
?>
<div class="d-flex woocommerce-checkout">
	<form class="woocommerce-cart-form w-50 checkout-half p-3" action="<?php echo esc_url(wc_get_cart_url()); ?>"
		  method="post">
		<?php do_action('woocommerce_before_cart_table'); ?>
		<h2 class="cart-product-total-header">Total: <?php echo $cart_count ?> Products</h2>
		<a href="<?php echo esc_url(wc_get_checkout_url()); ?>"
		   class="btn btn-primary mobile-checkout-btn checkout-button wc-forward mobile-only">
			<?php esc_html_e('Proceed to checkout', 'woocommerce'); ?>
			<span>(<?php echo $currency . ' ' . $price ?>)</span>
		</a>
		<table class="woocommerce-cart-form__contents">
			...
			foreach (WC()->cart->get_cart() as $cart_item_key => $cart_item) {
				...
				if ($_product && $_product->exists() && $cart_item['quantity'] > 0 && apply_filters('woocommerce_cart_item_visible', true, $cart_item, $cart_item_key)) {
					...
					?>
					<tr class="woocommerce-cart-form__cart-item cart-products-table <?php echo esc_attr(apply_filters('woocommerce_cart_item_class', 'cart_item', $cart_item, $cart_item_key)); ?>">

						<td class="product-thumbnail cart-product-thumbnail-section">
				...
				?>
						...
						<td class="product-name cart-product-info-section"
							data-title="<?php esc_attr_e('Product', 'woocommerce'); ?>">
							<h2 class="product-name desktop-only"
								data-title="<?php esc_attr_e('Product', 'woocommerce'); ?>">
								<?php
								if (!$product_permalink) {
									echo wp_kses_post(apply_filters('woocommerce_cart_item_name', $_product->get_name(), $cart_item, $cart_item_key) . '&nbsp;');
				...
				?>
						...
						<td class="product-remove cart-remove-product-section">
	...
	<div class="cart-collaterals w-50 checkout-half p-3">
		<?php
		/**
